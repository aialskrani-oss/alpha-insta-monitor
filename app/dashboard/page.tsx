...
import type { DashboardStats, Comment, FollowerChange } from '@/types'
...
export default function DashboardPage() {
  ...
  // بيانات رسم تغيرات المتابعين
  const changesChart = [...followerChanges]
    .reverse().slice(-15)
    .map(c => ({
      ...
      <div className="glass rounded-xl border border-cyber-border overflow-hidden">
        <button onClick={handleToggleChanges} className="w-full flex items-center justify-between p-4 hover:bg-white/[0.02] transition-colors">
          <div className="flex items-center gap-2">
            ...
            {followerChanges.length > 0 && (
              <span className="bg-blue-500/20 text-blue-400 text-xs rounded-full px-2 py-0.5">{followerChanges.length}</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {followerChanges.length > 0 && (
        ...
        {changesOpen && (
          <div className="border-t border-cyber-border p-4 space-y-4">
              ...
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={changesChart} margin={{ top: 4, right: 8, bottom: 4, left: 0 }}>
                  ...
                  <Bar dataKey="تغيير" radius={[4, 4, 0, 0]}>
                    {changesChart.map((c, i) => (
                      <Cell key={i} fill={(c.تغيير as number) >= 0 ? '#22c55e' : '#ef4444'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            {/* جدول */}
            {changesLoading ? (
              <div className="flex items-center justify-center py-6 text-cyber-muted gap-2">
                <Loader2 size={15} className="animate-spin" /><span className="text-sm">جارٍ التحميل…</span>
              </div>
            ) : followerChanges.length === 0 ? (
              <div className="py-8 text-center">
                ...
                <p className="text-xs text-gray-600 mt-1">اضغط <strong className="text-blue-400">فحص التغييرات</strong> لكشف التغييرات</p>
              </div>
            ) : (
              <div className="divide-y divide-cyber-border">
                  ...
                  <div key={c.id} className="flex items-center gap-3 py-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-bold ${c.change > 0 ? 'text-green-400' : 'text-red-400'}`}>
                          {c.change > 0 ? '+' : ''}{c.change.toLocaleString('ar')}
                        </span>
                ...
                {followerChanges.length > 10 && (
                  <p className="text-xs text-center text-cyber-muted pt-2">
                    + {followerChanges.length - 10} تغيير آخر
                  </p>
                )}
              </div>
            )}
      ...
      <div className="glass rounded-xl border border-cyber-border overflow-hidden">
        <button onClick={handleToggleComments} className="w-full flex items-center justify-between p-4 hover:bg-white/[0.02] transition-colors">
          <div className="flex items-center gap-2">
            ...
            {comments.length > 0 && (
              <span className="bg-ig-purple/20 text-ig-purple text-xs rounded-full px-2 py-0.5">{comments.length}</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {comments.length > 0 && (
        ...
        <div className="glass rounded-xl p-5 border border-cyber-border">
          ...
          <div className="space-y-3">
            {(stats?.topAccounts ?? []).map((account, idx) => {
              const acc = account as typeof account & { isPrivate?: boolean; avatar?: string }
